...
- `manual_daily_records`: a logged-in user can post their own production count and four capped (0-10h) hour categories for one calendar day via `POST /api/manual-daily-records`. Managers can also import one reporting date for active team members from the controlled `.xlsx` template via `/api/manual-daily-records/bulk-upload`; uploads validate every row before making any changes. Records are unique per (user, date), and edits reset prior review decisions to `pending`. Managers approve or reject records through `/api/manual-daily-records/<id>/approve|reject`. `GET /api/manual-daily-records` supports date, user, group, exclusion, and status filters.
